...
static uint8_t local_stack_mem[S3_MAX_TASKS * S3_TASK_STACK] __attribute__((aligned(4)));
...
static void draw_text(int px, int py, const char* s, uint16_t fg, uint16_t bg) {
    while (*s) { draw_char8(px, py, *s++, fg, bg); px += 8; }
}
...
static void draw_content_dirty(Win* w) {
    for (int i = 0; i < 3; i++) {
        if (memcmp(w->line[i], w->prev[i], 30) != 0) {
            draw_text(w->x+4, w->y+CONTENT_Y+i*LINE_H, w->line[i], C_WHITE, C_BLACK);
...
static void draw_frame(Win* w) {
    ...
    fill_rect(w->x+1, w->y+TITLE_H, w->x+w->w-2, w->y+w->h-2, C_BLACK);
    fill_rect(w->x+1, w->y+1, w->x+w->w-2, w->y+TITLE_H-1, w->title_bg);
...
int main(void) {
    ...
    _iocs_crtmod(13);              /* 320x256 65536-color */
    _iocs_g_clr_on();
    _iocs_b_curoff();
    _iocs_skeyset(0);
    ...
    _iocs_ms_curon();
    _iocs_ms_limit(0, SW-1, 0, SH-1);

    wins[0] = (Win){ 30, 15, WIN_W, WIN_H, C_BORDER0, C_TITLE0, "Timer",    {{{0}}},{{0}}};
    wins[1] = (Win){180, 60, WIN_W, WIN_H, C_BORDER1, C_TITLE1, "Keyboard", {{{0}}},{{0}}};
    wins[2] = (Win){ 80,120, WIN_W, WIN_H, C_BORDER2, C_TITLE2, "Mouse",    {{{0}}},{{0}}};
    ol_valid = 0;
    ...
    for (;;) {
        ...
        {   uint32_t sec  = frame / 57;
            ...
            sprintf(wins[0].line[0], "Vsync: %lu",  frame);    pad(wins[0].line[0],24);
        ...
        if (need_full) {
            fill_rect(0, 0, SW-1, SH-1, C_BG);
            for (int i = 0; i < 3; i++) {
                int idx = zmap[i];
                if (drag >= 0 && idx == drag) continue;
                draw_frame(&wins[idx]);
                memset(wins[idx].prev, 0xFF, sizeof(wins[idx].prev));
                draw_content_dirty(&wins[idx]);
            }
            if (drag >= 0) {
                ol_save(wins[drag].x, wins[drag].y,
                        wins[drag].w, wins[drag].h);
                draw_outline(wins[drag].x, wins[drag].y,
                             wins[drag].w, wins[drag].h);
            }
            need_full = 0;
        } else if (drag >= 0) {
            ...
            for (int i = 0; i < 3; i++)
                draw_content_dirty(&wins[zmap[i]]);
        }
    }
