...
static int order_to_index(uint8_t order) {
    return order - S3_BUDDY_MIN_ORDER;
}
...
static uint32_t block_index(void* ptr) {
    return ((uint8_t*)ptr - (uint8_t*)buddy.base) >> S3_BUDDY_MIN_ORDER;
}
...
void s3_mem_init(void* base, uint32_t size) {
    ...
    size &= ~((uint32_t)order_size(S3_BUDDY_MIN_ORDER) - 1);
    buddy.map_entries = size >> S3_BUDDY_MIN_ORDER;
    ...
    uint32_t usable_start = ((map_size + order_size(S3_BUDDY_MIN_ORDER) - 1)
                             & ~(order_size(S3_BUDDY_MIN_ORDER) - 1));
    uint32_t usable_size = size - usable_start;
    uint8_t* usable_base = (uint8_t*)base + usable_start;

    /* Find largest order that fits */
    uint8_t max_order = S3_BUDDY_MAX_ORDER;
    while (max_order > S3_BUDDY_MIN_ORDER && order_size(max_order) > usable_size) {
        max_order--;
    }

    /* Split from top order down */
    for (i = 0; i < S3_BUDDY_ORDERS; i++) {
...
void* s3_alloc(uint32_t size) {
    ...
    uint8_t order = S3_BUDDY_MIN_ORDER;
    while (order_size(order) < size && order <= S3_BUDDY_MAX_ORDER) {
        order++;
    }
    if (order > S3_BUDDY_MAX_ORDER) return NULL;

    int idx = order_to_index(order);

    /* Find a free block of sufficient order */
    uint8_t found_order = order;
    while (found_order <= S3_BUDDY_MAX_ORDER) {
        int fidx = order_to_index(found_order);
        if (buddy.free_lists[fidx] != NULL) break;
        found_order++;
    }
    if (found_order > S3_BUDDY_MAX_ORDER) return NULL;
...
void s3_free(void* ptr) {
    ...
    while (order < S3_BUDDY_MAX_ORDER) {
...
uint32_t s3_mem_free_bytes(void) {
    uint32_t total = 0;
    for (int i = 0; i < S3_BUDDY_ORDERS; i++) {
