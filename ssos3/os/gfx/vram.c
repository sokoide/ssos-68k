...
void s3_dma_init(void) {
    ...
    dma.csr = (volatile uint8_t*)(S3_DMA_CH2_BASE + 0x00);
    dma.ccr = (volatile uint8_t*)(S3_DMA_CH2_BASE + 0x07);
    dma.mar = (volatile uint32_t*)(S3_DMA_CH2_BASE + 0x0C);
    dma.dar = (volatile uint32_t*)(S3_DMA_CH2_BASE + 0x14);
    dma.mtc = (volatile uint16_t*)(S3_DMA_CH2_BASE + 0x0A);
    dma.niv = (volatile uint8_t*)(S3_DMA_CH2_BASE + 0x25);
}
...
void s3_gfx_init(void) {
    vram.draw_page = S3_GVRAM_PAGE0;
    vram.display_page = S3_GVRAM_PAGE1;
    vram.draw_idx = 0;
    vram.display_idx = 1;
...
void s3_gfx_clear(uint16_t color) {
    ...
    uint32_t page_size = S3_PAGE_SIZE;
    s3_dma_fill((volatile void*)vram.draw_page, color, (uint16_t)(page_size / 2));
}
...
void s3_gfx_rect(int x, int y, int w, int h, uint16_t color) {
    ...
    if (x < 0) { w += x; x = 0; }
    if (y < 0) { h += y; y = 0; }
    if (x + w > S3_SCREEN_W) w = S3_SCREEN_W - x;
    if (y + h > S3_SCREEN_H) h = S3_SCREEN_H - y;
    if (w <= 0 || h <= 0) return;
    ...
    for (int row = y; row < y + h; row++) {
        volatile uint32_t* row_ptr = (volatile uint32_t*)
            ((uint8_t*)vram.draw_page + row * S3_BYTES_PER_LINE + (x / 16) * S3_PLANES * 2);
        int words = (w + 15) / 16;
