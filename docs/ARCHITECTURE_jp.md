...
graph TB
    ...
    subgraph "CLI機能"
        I[コマンドライン<br/>インターフェース] --> J[入力処理<br/>システム]
        J --> K[コマンド<br/>実行]
        K --> L[出力<br/>表示]
    end
```
...
### 3. CLIシステム (`ssos/os/main/`)

**CLIシステム:**
```c
typedef struct {
    char command_buffer[256];  // コマンドバッファ
    int buffer_pos;            // バッファ位置
    char command_history[16][256];  // コマンド履歴
    int history_count;         // 履歴数
