...
| :--- | ---: | ---: | ---: | ---: |
| full | 1020 | 1393 | +36.6% | 29,671,600 → 42,778,800（+44.2%） |
| region | 188 | 188 | 0% | 2,919,200 → 2,919,200 |
| z-expose | 416 | 417 | +0.2% | 8,497,200 → 8,497,200 |
| text-update | 15 | 15 | 0% | 112,000 → 112,000 |
| xor-move | 26 | 25 | -3.8% | read/writeとも116,352 |

DMA方向とアラインメントを修正した後の確定値である。両モードとも `dma ok` が全試行で、`error=0`、`timeout=0`、`fallback_rows=0` になっている。16色モードは `full` で256色より36.6%遅く、VRAM書き込みも44.2%多いため、採用しない。`region` と `z-expose` がほぼ同値なのは、今回のテスト経路ではモード差が測定対象の論理書き込み量に現れていないためであり、16色モード全体が高速という意味ではない。
...
5. 重なりが多いケースをベンチに追加し、領域再合成で描くウィンドウ数と描画量を測定する。
