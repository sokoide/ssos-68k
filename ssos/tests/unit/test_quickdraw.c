...
TEST(quickdraw_shell_info_panel_initial_draw) {
    ...
    uint8_t block_font[256 * 16];
    memset(block_font, 0xFF, sizeof(block_font));
    qd_set_font_bitmap(block_font, 8, 16);

    qd_shell_init_info_panel();

    ASSERT_EQ(qd_get_pixel(16, 80), QD_COLOR_BLACK);
    ASSERT_EQ(qd_get_pixel(16 + 512 - 12, 80 + 4), QD_COLOR_GREEN);
    ASSERT_EQ(qd_get_pixel(16 + 512 - 12, 80 + 24 + 40), QD_COLOR_BRIGHT_WHITE);

    qd_shell_update_info_panel(false);
    ASSERT_EQ(qd_get_pixel(16 + 512 - 12, 80 + 24 + 40), QD_COLOR_BRIGHT_WHITE);
}
