...
void draw_stats(uint16_t* buf) {
    ...
    if (dt != prev_dt) {
        ...
        ss_print_v(buf, WIDTH, HEIGHT, color_fg, color_bg, 0, oy, szMessage);
    }
    ...
    if (pos != prev_pos) {
        ...
        sprintf(szMessage, "mouse x:%3d, y:%3d", (pos & 0xFFFF0000) >> 16,
                pos & 0x0000FFFF);
        ss_print_v(buf, WIDTH, HEIGHT, color_fg, color_bg, 0, oy + 16, szMessage);
    }
    ...
    if (counter++ >= 30) {
        ...
        sprintf(szMessage, "A: V-DISP counter: %9d (vsync count)",
                ss_timera_counter);
        ss_print_v(buf, WIDTH, HEIGHT, color_fg, color_bg, 0, y, szMessage);
        y += 16;
        ...
        // sprintf(szMessage, "ssp: 0x%08x, pc: 0x%08x, sr: 0x%04x", ssp, pc,
        // sr); ss_print(color_fg, color_bg, 0, y, szMessage); y += 16;
        ...
        // sprintf(szMessage, "ss_save_data_base addr: 0x%p",
        // &ss_save_data_base); ss_print(color_fg, color_bg, 0, y, szMessage);
        // y += 16;

        // sprintf(szMessage, "memory total: %d, free: %d",
        // ss_mem_total_bytes(),
        //         ss_mem_free_bytes());
        // ss_print(color_fg, color_bg, 0, y, szMessage);
        // y += 16;
