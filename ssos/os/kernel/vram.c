...
void ss_fill_rect_v(uint16_t* offscreen, uint16_t w, uint16_t h, uint16_t color,
    ...
    for (int y = y0; y <= y1; y++) {
        ...
        if (width >= 2 && ((uintptr_t)row_start & 3) == 0) {
            uint32_t* dst32 = (uint32_t*)row_start;
            int blocks = width >> 1;  // width / 2

            // Fill 2 pixels at once using 32-bit writes
            ...
            // Fallback for unaligned or small areas
...
void ss_draw_rect_v(uint16_t* offscreen, uint16_t w, uint16_t h, uint16_t color,
                    int x0, int y0, int x1, int y1) {
    ...
    if (width >= 2 && ((uintptr_t)top_line & 3) == 0) {
    ...
    if (y1 != y0) {
        uint16_t* bottom_line = &offscreen[y1 * w + x0];
        if (width >= 2 && ((uintptr_t)bottom_line & 3) == 0) {
...
void ss_put_char_v(uint16_t* offscreen, uint16_t w, uint16_t h,
...
void ss_print_v(uint16_t* offscreen, uint16_t w, uint16_t h, uint16_t fg_color,
...
int ss_print_v_smart(uint16_t* offscreen, uint16_t w, uint16_t h, uint16_t fg_color,
...
void ss_fill_rect_v_fast_aligned(uint16_t* offscreen, uint16_t w, uint16_t h,
    ...
    for (int y = y0; y <= y1; y++) {
        ...
        if (width > 0 && ((uintptr_t)row_start & 3)) {
            *row_start++ = color;
            width--;
        }
        ...
        if (width >= 2) {
            ...
            for (int i = 0; i < blocks; i += 4) {
                if (i + 4 <= blocks) {
                    // Process 4 blocks at once (8 pixels)
                    dst32[0] = color32;
                    dst32[1] = color32;
                    dst32[2] = color32;
                    dst32[3] = color32;
            ...
            row_start = (uint16_t*)dst32;
            width = width & 1;
        }

        // Handle remaining unaligned pixel
        if (width > 0) {
            *row_start++ = color;
        }
    }
