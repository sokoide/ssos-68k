...
| RAM | `0x00040000` 〜 | 64KB | `.data`+`.bss` 領域（`entry.s` 開始時の SP は `0x010000`）|
...
│   └── standalone/                      # スタンドアロン (.x) 用独自 main.c
...
- **スタンドアロンモード** (`.x`): `LOCAL_MODE` 定義ありで Human68K 実行形式としてコンパイル。`gfx`/`mem`/`kernel` コードを共有するが、独自の `standalone/main.c` と独自のウィンドウ/フレーム/ドラッグ実装を持つ。開発イテレーションが高速
- **OS モード** (`.xdf`): カスタムブートローダを持つ完全起動可能システム。スタンドアロンモードから切り替える際は `make clean` が必要

### ウィンドウ Z-Order 管理: OS モードがスタンドアロンと異なる理由
...
**OS 版がスタンドアロンの方式を流用しない理由:**

スタンドアロン版は 3 ウィンドウ固定で `zmap[]` を 3 要素の順序配列として使う — スロットインデックスがそのまま z 値となる。これは簡潔だがスケーラブルではない。
...
3. **`rebuild_zmap` でブロックレベル max-z を使用**（8×8 ブロック = 96×64 ブロックのオクルージョンマップ）: 各ウィンドウが z 値を持つことで成立する設計
4. **ドラッグによる前面移動で z を単調増加**: 順序配列のスキャンなしで、ドラッグされたウィンドウが確実に新たな最前面になる

トレードオフとして、**単調増加 z は初期状態に注意が必要**: 最初のドラッグが起動時の z 値と衝突してはいけない。OS 版は `next_z` を **初期 z の最大値より 1 大きい値**（z=1..3 で作成したウィンドウに対して `next_z = 4`）に設定することで、2 つのウィンドウが同一 z 値（両方アクティブ表示になる）を共有するのを防いでいる。255 到達後は折り返し。詳細: [知見 > s29](#s29-初回ドラッグの-z-衝突解決済み)。

### `.x` ビルドでリンクされるオブジェクト

`ssos-cooperative/standalone/Makefile` の `SRCS` から:
...
`os/app/main.c` および `os/win/window.c` は含まれない。よって `os/app/main.c` / `os/win/window.c` / `os/win/win.h` の変更は **スタンドアロンに無影響**。
...
- **スタンドアロンにこのバグがない理由**: 3 スロットの順序配列 `zmap[3]={0,1,2}` と `bring_to_front(idx)` を使っている — 配列スロットは衝突しない
- **修正**: `next_z` を `4` に初期化（初期 z の最大値より 1 大きい値）。255 到達後は `3` に折り返し（最初のドラッグで z=3 を踏むリスクは、255 回到達時には許容する設計判断）
...
| **Z-order の 255 到達後折り返し** | 長いセッションで `next_z=255 → 3` に戻り、初回ドラッグと類似の z 衝突が起きる可能性。要リファクタリング案: z=255 ではなく hid の最低 z を探し再採番 |
