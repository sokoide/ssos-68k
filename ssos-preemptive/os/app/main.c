...
static void update_content(uint16_t wt, uint16_t wk, uint16_t wm,
    ...
    sprintf(t->line[1], "Tick: %lu", (unsigned long)ss_tick_counter);
    t->line[2][0] = '\0';
...
void ss_run(void) {
    ...
    /* Main loop — preemptive: Timer D ISR switches contexts, no yield */
    while (1) {
        wait_vsync();
        frame++;
        update_mouse();
        ...
        if (left && drag_id < 0) {
            ...
            if (hid > 0) {
                ...
                ss_win_hide(hid);
                ss_win_render_region(ox, oy, drag_w, drag_h);
